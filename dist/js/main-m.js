...
*/
		...
		city_ya_detect : function() {
			...
			if(utm_source) $.cookie('utm_source', utm_source);

			var utm_campaign = App.Func.get_url_param("utm_campaign");
			if(utm_campaign) $.cookie('utm_campaign', utm_campaign);

			var utm_content = App.Func.get_url_param("utm_content");
			if( utm_content ) $.cookie('utm_content', utm_content);

			var utm_term = App.Func.get_url_param("utm_term");
			if( utm_term ) $.cookie('utm_term', utm_term);
			
			// таймзон
			var gmtHours = -new Date().getTimezoneOffset()/60;
			if(gmtHours) $.cookie('gmtHours', gmtHours);
			
			if( YMaps.location.city ) {
				$.cookie('city', YMaps.location.city);
				$.cookie('country', YMaps.location.country);
				$.cookie('region', YMaps.location.region);
				$('input[name="city"]').val( YMaps.location.city ); 
			}
			
/*
			if( YMaps.location.country=="Казахстан" ) {
				$('span.line1').html('Трое часов за 17900 тенге (-50%)');
				$('.zakaz').html('12790 тенге <s>19200 тенге</s>.');
				$('.dostavka').html('1290 тенге');
			}
*/
